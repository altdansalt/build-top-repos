...
**89 projects landed, 32 deferred** (see ledger). Six cached language toolchains:
...
| 12 | openclaw | TS/pnpm | — | — | — | ⏸️ deferred |
...
| 16 | vscode | TS | — | — | — | ⏸️ deferred |
| 14 | pytorch | Python | — | — | — | ⏸️ deferred |
| 9 | posthog | Python | — | — | — | ⏸️ deferred |
...
**vscode — deferred (Electron GUI code editor; no meaningful headless mode; multi-GB build with native C++ addons).** Visual Studio Code is a full Electron desktop application (rank 16 — one of the largest codebases in the dataset). Its TypeScript build (`yarn install && yarn compile`) requires the Electron binary (downloaded separately via `@electron/get`, not pinned in our toolchain) and several native C++ Node.js addons (`node-pty`, `vscode-node-ripgrep`, `vscode-nsfw`, `vscode-proxy-agent`) that need a C++ compiler not present in `node_rootfs`. The compiled JavaScript must be run as `electron .` — there is no standalone headless CLI in the TypeScript layer. Even `code --version` invokes the Electron binary, which requires a display connection on Linux. The companion project `code-server` (rank 725) was already deferred because its own build compiles the bundled `microsoft/vscode` submodule as a multi-GB, long build — vscode itself is the same obstacle. A dedicated `electron_rootfs` toolchain baking a pinned Electron binary and the required C++ addon headers would be needed for any honest smoke. Same category as electron, tabby, lossless-cut, and alacritty.

**openclaw — deferred (Node.js 22.19+ required; pinned toolchain has 22.14.0).** openclaw is a TypeScript/pnpm AI-assistant CLI (rank 12). Its entry point (`openclaw.mjs`) calls `ensureSupportedNodeVersion()` before any argument parsing — including `--version` — and exits with code 1 when the running Node major.minor is below 22.19. Our `node_rootfs` toolchain pins Node 22.14.0, which fails this check. The build pipeline (pnpm monorepo + tsdown bundler + multiple postbuild scripts) would likely succeed if attempted, but there is no honest smoke possible without a Node 22.19+ runtime. Upgrading the `node_rootfs` to Node 22.19 (or the current LTS release) is the right investment — deferred until that toolchain is updated.

**pytorch — deferred (C++ build requires hours even for CPU-only; no honest build-from-source path in current toolchains).** PyTorch is a deep learning framework whose Python package is backed by thousands of C++ extension files compiled via cmake + ninja + g++, along with native deps including OpenBLAS/LAPACK, pybind11, Eigen, protobuf, and more. Even a CPU-only build (`USE_CUDA=0 python setup.py develop`) typically takes 1–4 hours on fast hardware, far beyond our Bazel genrule timeout. A GPU-accelerated build additionally requires a `cuda_rootfs` toolchain holding the NVIDIA CUDA toolkit. The only fast alternative — installing the pre-built CPU wheel from PyTorch's whl index (`pip install torch --index-url https://download.pytorch.org/whl/cpu`) — downloads a binary artifact not built from the pinned source commit, making it an unprincipled smoke (equivalent to downloading a pre-built `python` binary instead of `./configure && make` for cpython). Deferred until we invest in a purpose-built pytorch toolchain pre-baking all native deps, or a mechanism to pin a PyTorch wheel release to a corresponding source commit.

**posthog — deferred (full-stack web analytics platform; multiple required external services; no standalone CLI).** PostHog (rank 9 — one of the largest codebases in the list) is an all-in-one product analytics platform whose primary component is a Django web application requiring PostgreSQL, ClickHouse, Redis, and Kafka to start any process. The repo is a complex polyglot monorepo: a Python/Django backend, a React/TypeScript frontend, a Node.js plugin server (`plugin-server/`), Rust services (`rust/`), and Go services (`go/`) — each requiring a separate toolchain, with no single `python_rootfs` able to cover the whole build. Even installing the Python layer alone (`pip install -e .`) pulls heavy Django/Celery/Kafka/ClickHouse deps, and the resulting package has no CLI entry point that runs without a live database — `manage.py` commands, Celery workers, and the web server all call `django.setup()` which connects to the database before any command dispatch. There is no `posthog --version` or lightweight headless operation mode. Deferred pending a full-stack service-integration strategy.
